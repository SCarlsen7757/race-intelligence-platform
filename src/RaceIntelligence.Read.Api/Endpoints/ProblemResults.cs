...
internal static class ProblemResults
{
    ...
    /// <summary>404: the session exists but recorded no telemetry for that lap.</summary>
    /// <remarks>
    ...
    /// </remarks>
    public static IResult LapNotFound(Guid sessionId, int lapNumber) => Results.Problem(
        statusCode: StatusCodes.Status404NotFound,
        title: "Lap not found",
        detail: $"Session '{sessionId}' has no telemetry for lap {lapNumber}.");
    ...
    /// nothing to say so.
    /// </remarks>
    public static IResult LapTooLarge(int count, int maximum) => Results.Problem(
        statusCode: StatusCodes.Status400BadRequest,
        title: "Lap too large to read",
        detail: $"That lap holds {count} samples; this endpoint serves at most {maximum} at a time.");
}
