...
import { WHEELS } from '../../live/contracts';
import { formatNumber, formatPercent } from '../../live/format';
...
function WheelGrid({
  ...
  read: (index: number) => (frame: import('../../live/contracts').FocusFrameMessage) => number | null | undefined;
 ...
 * capability it needs, so none of them is chosen by asking which game this is.
 *
 * The RaceRoom-specific extras the dashboard will eventually want — push-to-pass, DRS, virtual
 * energy, cut-track warnings, tyre subtype, pit menu state — are deliberately absent: none of them
 * is on the live wire yet. Adding them is connector and contract work, not frontend work, and is
 * additive to the MessagePack keys when it happens.
 */
