...
import { formatGear, formatNumber, formatSpeed } from '../live/format';
 ...
 *
 * Everything below the header is painted outside React — see `PedalTrace` and `LiveReadout`. This
 * component re-renders only when the driver, the room, or the available panels change.
 */
export function FocusPanel({ driverKey, gameKey, capabilities, onClose }: FocusPanelProps) {
  ...
  const renderSpeed = useCallback(
    (frame: import('../live/contracts').FocusFrameMessage) =>
      formatSpeed(frame.speedMetersPerSecond),
    [],
  );
    ...
    <aside className="focus">
      ...
      {panels.length === 0 && (
        <p className="focus__note">
          This collector reports no channels beyond the basics above.
        </p>
      )}
    </aside>
  );
