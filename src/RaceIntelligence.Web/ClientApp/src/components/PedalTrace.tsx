...
/**
 * Throttle, brake and steering over the last minute, painted at the display's refresh rate.
 *
...
export function PedalTrace({ store, height = 200 }: PedalTraceProps) {
  ...
  useEffect(() => {
    ...
    const chart = new uPlot(
      {
        ...
        axes: [
          { show: false },
          { scale: 'pedal', stroke: '#8b93a7', grid: { stroke: '#1e2433' } },
        ],
        legend: { show: false },
        cursor: { show: false },
        series: [
          {},
          { label: 'Throttle', scale: 'pedal', stroke: '#3ddc84', width: 1.5, spanGaps: false },
          { label: 'Brake', scale: 'pedal', stroke: '#ff5c5c', width: 1.5, spanGaps: false },
          { label: 'Steering', scale: 'steer', stroke: '#5aa9ff', width: 1, spanGaps: false },
        ],
      },
      [new Float64Array(0), new Float64Array(0), new Float64Array(0), new Float64Array(0)],
      container,
    );
    ...
    const paint = () => {
      ...
      chart.setData([xs, throttle, brake, steering], true);
