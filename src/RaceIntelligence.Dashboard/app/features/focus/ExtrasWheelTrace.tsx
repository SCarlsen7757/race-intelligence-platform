...
import { TYRE_TRACE_CAPACITY, type ExtrasTraces, type WheelTraces } from '../../shared/live/store';
...
/**
 * Which extras ring a four-wheel chart draws, and where the same channel is read for its readouts.
 *
 ...
 */
export interface ExtrasWheelChannel {
  ring: (extras: ExtrasTraces) => WheelTraces;
  read: (document: RaceRoomExtras | null, wheel: number) => number | undefined;
  /**
   * The simulator's window for this channel, where it reports one.
   *
   * Takes the whole document rather than one wheel's reading because a window is a property of the
   * compound or the pad rather than of a corner — see {@link firstReportedWindow}.
   */
  window?: (document: RaceRoomExtras | null) => OperatingWindowValues | null;
}

interface ExtrasWheelTraceProps extends ChannelPanelProps {
  channel: ExtrasWheelChannel;
  unit: string;
...
/**
 * One extras channel over a stint, four wheels on one axis, with the current value beside each.
 *
 * The extras counterpart of `WheelTrace`, and separate from it only because the two read from
 * different places: `WheelTrace` draws the typed frame's tyre rings at focus cadence, this draws
 * the extras rings at roughly one sample a second. Everything above that — four corners on one
 ...
 */
export function ExtrasWheelTrace({
  store,
  driverKey,
  hiddenChannels,
  onToggleChannel,
  ...
  const spec: LiveChartSpec = {
    ...
    ...(readWindow === undefined
      ...
      : {
          band: {
            // Read from the store rather than from the `extras` above, so the band survives a chart
            // built between renders — and so the closure does not pin the document this render saw.
            read: () => readWindow(store.getExtras()[driverKey]?.document ?? null),
          },
        }),
  };
  ...
  return (
    <div className="wheel-chart">
      ...
      <ChannelLegend
        ...
        unit={unit}
        renderValue={(_, index) => {
          const value = reportedNumber(channel.read(extras?.document ?? null, index));
