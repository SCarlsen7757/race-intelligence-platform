...
export function PedalBars({ store, driverKey, height = 108 }: PedalBarsProps) {
  ...
  useEffect(() => {
    ...
    const paint = () => {
      ...
      const totalWidth = PEDALS.length * BAR_WIDTH + (PEDALS.length - 1) * BAR_GAP;
      ...
      PEDALS.forEach((pedal, index) => {
        ...
        if (Number.isNaN(value)) {
          paintHatch(x, 0, BAR_WIDTH, barHeight);
        } else {
          const clamped = Math.min(1, Math.max(0, value));
          const filled = Math.round(barHeight * clamped);
          context.fillStyle = pedal.colour;
          context.fillRect(x, barHeight - filled, BAR_WIDTH, filled);
        }

        context.fillStyle = TRACE_COLOURS.axis;
        context.fillText(pedal.label, x + BAR_WIDTH / 2, barHeight + 3);
      });
