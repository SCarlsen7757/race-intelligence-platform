...
public sealed class LiveOutboxTests
{
    ...
    [Fact]
    public void Slow_frames_conflate_into_one_slot_like_the_other_data_frames()
    {
        var outbox = CreateOutbox();
        var sessionId = Guid.NewGuid();

        outbox.PublishSlowChannels(SlowSample(sessionId, damageEngine: 1.0f), OperatingWindowFactory.Create(), DateTimeOffset.UnixEpoch, "4242");
        outbox.PublishSlowChannels(SlowSample(sessionId, damageEngine: 0.5f), OperatingWindowFactory.Create(), DateTimeOffset.UnixEpoch, "4242");

        outbox.TryRead().ShouldBeOfType<LiveSlowFrame>().Sample.DamageEngine.ShouldBe(0.5f);
        outbox.TryRead().ShouldBeNull();
    }

    /// <summary>A sample carrying one recognisable slow channel, so a conflated frame can be identified.</summary>
    private static RaceRoomTelemetrySample SlowSample(Guid sessionId, float damageEngine) =>
        TelemetrySampleFactory.Create(sessionId) with { DamageEngine = damageEngine };

    /// <summary>
    /// The acceptance criterion for the slow channel: switching it on must not cost the focus
    /// stream anything. Slow frames sit below self in the ladder, so a self frame is never held back
    /// behind a once-a-second one — and because both are single slots, a slow frame cannot displace
    /// a self frame either.
    /// </summary>
    [Fact]
    public void Slow_frames_are_delivered_last_so_the_focus_stream_keeps_its_rate()
    {
        var outbox = CreateOutbox();
        var sessionId = Guid.NewGuid();
        ...
        outbox.TryRead().ShouldBeOfType<LiveStandingsFrame>();
        outbox.TryRead().ShouldBeOfType<LiveSelfFrame>();
        outbox.TryRead().ShouldBeOfType<LiveSlowFrame>();
    }

    /// <summary>
    /// Every self frame published between two reads still arrives — one per read — with a slow
    /// frame outstanding the whole time. A slow frame being outstanding must not cost the focus
    /// stream a single frame, which is what "the rate is unaffected" means concretely.
    /// </summary>
    [Fact]
    public void An_outstanding_slow_frame_never_displaces_a_self_frame()
    {
        var outbox = CreateOutbox();
        var sessionId = Guid.NewGuid();
        ...
        outbox.DroppedFrames.Self.ShouldBe(0, "nothing about the slow channel causes a self frame to be superseded");
        outbox.TryRead().ShouldBeOfType<LiveSlowFrame>();
    }
